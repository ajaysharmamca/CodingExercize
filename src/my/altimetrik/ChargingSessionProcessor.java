...
/**
 * EV Charging Session Event Processor
 * - Virtual threads for ingestion
 * - Partitioned single-thread executors for ordering
 */
public class ChargingSessionProcessor {
    ...
    private final ExecutorService[] partitions =
            new ExecutorService[PARTITIONS];
    ...
    // ---- Constructor ----
    public ChargingSessionProcessor() {
    ...
    public void submitEvent(SessionEvent event) {
        dispatcher.submit(() -> routeToPartition(event));
    }

    // ---- Routing Logic ----
    private void routeToPartition(SessionEvent event) {
    ...
    private void processOrdered(SessionEvent event) {
        ...
        latestStatus.compute(event.sessionId(), (key, existing) -> {
            ...
            // Simple state validation (optional but realistic)
    ...
    public static void main(String[] args) throws Exception {
        ...
        ChargingSessionProcessor processor = new ChargingSessionProcessor();
        ...
        List<SessionEvent> events = List.of(
                ...
                new SessionEvent("S1", 5, Status.STARTED) // ignored (older)
        );
